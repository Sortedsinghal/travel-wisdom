...
const TripCard: React.FC<TripCardProps> = ({
  ...
  return (
    <div className="px-1 md:px-2 pb-2 md:pb-4 h-full">
      <Link to={`/trip/${slug}`} className="block h-full">
        <div className="flex flex-col h-full bg-white rounded-xl md:rounded-2xl shadow-md overflow-hidden transition-transform duration-300 hover:-translate-y-1 hover:shadow-lg cursor-pointer">
          ...
          <div className="flex flex-col justify-between flex-grow p-2 md:p-4 mt-1">
            <article>
              <p className="text-sm md:text-base lg:text-lg font-semibold text-gray-800 min-h-[2.5rem] md:min-h-[3rem] line-clamp-2 leading-relaxed">{title}</p>
            ...
            <div className="flex items-center gap-1 md:gap-2 mt-2 md:mt-4">
              <span className="flex-1 text-center bg-blue-100 text-blue-800 font-semibold py-1.5 md:py-2 px-2 md:px-3 rounded-lg text-xs md:text-sm whitespace-nowrap">
                Details
              </span>
              <button
                onClick={(e) => {
