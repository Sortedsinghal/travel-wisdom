...
const PopupForm: React.FC<PopupFormProps> = ({ isOpen, onClose, trip }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    fetch(`${process.env.NODE_ENV === 'production' ? '/api' : 'http://localhost:4000/api'}/send-query`, {
      method: 'POST',
      headers: {
  ...
  return (
    <>
      ...
      {/* Main Modal Overlay */}
      <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center z-[9999] p-2 sm:p-4 font-inter transition-opacity duration-300">
          ...
          <div className="hidden md:flex md:w-1/2 bg-[#0B3A55] text-white p-3 sm:p-6 rounded-t-lg sm:rounded-t-2xl md:rounded-l-2xl md:rounded-tr-none flex-col items-center justify-center space-y-1 sm:space-y-3 relative overflow-hidden">
            ...
            {trip && (
              <div className="z-10 mt-2 sm:mt-4 p-1.5 sm:p-2 bg-white bg-opacity-10 rounded-lg">
                  <p className="text-xs sm:text-sm font-semibold">Viewing Itinerary for:</p>
                  <p className="text-xs sm:text-base lg:text-lg font-bold text-[#5B92A7]">{trip.title}</p> 
              </div>
            )}
          </div>
