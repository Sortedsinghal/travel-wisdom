        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
            {visibleTrips.map((trip) => (
              <div key={trip.slug} onClick={() => navigate(`/trip/${trip.slug}`)} className="cursor-pointer">
                  ...
                  <CardContent className="p-2 md:p-6 pt-0 flex-1 flex flex-col">
                    <div className="mb-2 md:mb-3 flex-1">
                      {trip.price.startsWith('₹') ? (
                        <>
                          <span className="font-bold text-sm md:text-lg text-gray-900">{trip.price}</span>
                          {trip.originalPrice && (
                            <span className="text-xs md:text-sm text-gray-500 line-through ml-2">{trip.originalPrice}</span>
                          )}
                          <span className="text-xs md:text-sm text-gray-600 block">per person</span>
                        </>
                      ) : (
                        <span className="text-xs md:text-sm text-blue-600 font-medium">{trip.price}</span>
                      )}
                    </div>
